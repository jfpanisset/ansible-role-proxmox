...
pve_domains_cfg: {} # List of realms to use as authentication sources in the PVE domains.cfg config eil.
```
...
You can set realms / domains as authentication sources in the `domains.cfg` configuration file. If this file is not present, only the `Linux PAM` and `Proxmox VE authentication server` are available. Supported types are `pam`, `pve`, `ad` and `ldap`. One domain should have the `default: 1` property to mark it as the default: 
